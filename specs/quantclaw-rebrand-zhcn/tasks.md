...
- 统一 Web 页面品牌图引用到正式资源
- 评估是否单独生成 favicon 资源
- 将阶段三当前收口定义为“Web 资源已完成”，不等待桌面端素材

### 3.2 Tauri 资源替换

- 当前后置，不纳入本轮阶段三执行
- 后续待用户提供桌面应用图标素材后，再替换以下资源：
...
- 设计 daemon / channels orchestrator 侧的安全实现方案
- 明确 gateway 仅承担元数据与入口引导，不直接承载微信二维码运行逻辑
- 产出二维码状态机、鉴权边界、限流与持久化方案，供后续单独实施

### 4.5 阶段四验证

- 用户可通过图形界面完成 `QQ` 配置
- 用户可通过图形界面完成 `企业微信` 配置
- 微信入口具备明确的后续接入说明与安全边界说明
